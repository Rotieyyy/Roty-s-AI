// Load persistent theme & UI style
(function() {
    const savedTheme = localStorage.getItem('roty-ai-theme') || 'dark';
    document.documentElement.setAttribute('data-theme', savedTheme);
    
    const savedUIStyle = localStorage.getItem('roty-ai-ui-style') || 'classic';
...
function highlightCodeBlocks(container) {
    ...
    container.querySelectorAll('pre code').forEach(code => {
        if (!code.dataset.highlighted) {
            ...
            // Wrap code element in wrapper container to align flex with gutter
...
auth.onAuthStateChanged(async (user) => {
    ...
    try {
        ...
        if (
            ...
            const wasGuest = localStorage.getItem('roty-ai-was-guest') === 'true';
            if (wasGuest) {
                useGuestMode();
            } else {
...
function useGuestMode() {
    ...
    isSharedView = false;
    currentUserUid = null;
    localStorage.setItem('roty-ai-was-guest', 'true');
    ...
    // Set as empty arrays to save storage space (conversations are transient in memory)
    allChats = [];
    userGallery = [];
...
function handleLogout() {
    localStorage.removeItem('roty-ai-was-guest');
    auth.signOut().then(() => {
        currentUserUid = null;
        allChats = [];
        userGallery = [];
...
function getLocalGuestChats() {
    return []; // Storage saving mode
}

function saveLocalGuestChats(chats) {
    // Disabled to save storage space
}

function getLocalGuestGallery() {
    return []; // Storage saving mode
}

function saveLocalGuestGallery(gallery) {
    // Disabled to save storage space
}
...
if (SpeechRecognition) {
    ...
    recognition.onstart = () => {
        ...
        document.getElementById('voice-waveform')?.classList.remove('hidden');
    };
...
function renderWelcomeHero() {
    const style = document.documentElement.getAttribute('data-ui-style') || 'classic';
    let cardsHtml = '';
    
    if (style === 'premium') {
        cardsHtml = `
            <div class="prompt-starters-grid">
                <div class="prompt-card" onclick="usePromptStarter('Find the bug in this asynchronous JavaScript fetch block:\\n\\n\`\`\`js\\nasync function getData() {\\n  let res = fetch(\\'/api/data\\');\\n  let data = res.json();\\n  return data;\\n}\\n\`\`\`')">
                    <div class="card-icon"><i class="fas fa-bug"></i></div>
                    <div class="card-text">
                        <h4>Debug JS Code</h4>
                        <p>Analyze async errors</p>
                    </div>
                </div>
                <div class="prompt-card" onclick="usePromptStarter('Brainstorm 5 unique, creative ideas for a viral technology YouTube video about future AI automation.')">
                    <div class="card-icon"><i class="fas fa-lightbulb"></i></div>
                    <div class="card-text">
                        <h4>Brainstorm Ideas</h4>
                        <p>Generate creative hooks</p>
                    </div>
                </div>
                <div class="prompt-card" onclick="usePromptStarter('Write a short, engaging sci-fi micro-story about an astronaut who discovers an ancient signal on Mars.')">
                    <div class="card-icon"><i class="fas fa-book"></i></div>
                    <div class="card-text">
                        <h4>Write a Story</h4>
                        <p>Draft engaging micro-fiction</p>
                    </div>
                </div>
                <div class="prompt-card" onclick="usePromptStarter('Provide a detailed, cinematic photography prompt for an image generator showing a neon-lit cyberpunk alleyway in Tokyo.')">
                    <div class="card-icon"><i class="fas fa-camera"></i></div>
                    <div class="card-text">
                        <h4>Photo Prompt</h4>
                        <p>Craft detailed image prompts</p>
                    </div>
                </div>
            </div>
        `;
...
async function sendMessage(text, options = {}) {
    ...
    try {
        ...
        try {
                        ...
                        if (dataStr && dataStr !== '[DONE]') {
                            try {
                                ...
                                if (aiIndex === null) {
                                    ...
                                    chat.msgs[aiIndex].text = accumulatedText;
                                }

                                renderAiMessage(aiNode, accumulatedText, aiIndex, { skipTools: true });
                            } catch (e) {
            ...
            if (aiIndex === null) {
                accumulatedText = "The AI returned an empty response.";
                aiIndex = await pushAiMessage(chat, accumulatedText, { engine, persona });
            }

            renderAiMessage(aiNode, accumulatedText, aiIndex);
            await persistChat(chat);
...
function renderUserMessage(div, content, messageIndex) {
    ...
    if (messageIndex !== null && !isSharedView) {
        ...
        const timeString = message.createdAt ? formatMessageTime(message.createdAt) : "";
...
function downloadMessage(rawText, bodyElement, format) {
    ...
    if (format === 'md') {
        downloadBlob(rawText, 'text/markdown;charset=utf-8', `${baseName}.md`);
...
function renderHistory(query = '') {
    ...
    const guestMsg = document.getElementById('guest-history-msg');
    ...
    if (sidebarSearch) sidebarSearch.style.display = 'flex';
    ...
    filteredChats.forEach((c, index) => {
        const originalIndex = allChats.findIndex(chat => chat.id === c.id);
...
async function saveImageToGallery(imageUrl, promptText) {
    ...
    if (isGuest) {
        ...
        saveLocalGuestGallery(userGallery);
...
function toggleTheme() {
    ...
    localStorage.setItem('roty-ai-theme', theme);
}

// --- SETTINGS MODAL & UI STYLE ---
...
function setUIStyle(style) {
    ...
    localStorage.setItem('roty-ai-ui-style', style);
    
    updateSettingsModalUI();
    
    // Update welcome screen if it's currently showing
    const welcome = document.getElementById('welcome-hero');
    if (welcome) {
...
function updateSettingsModalUI() {
    const style = document.documentElement.getAttribute('data-ui-style') || 'classic';
    const btnClassic = document.getElementById('btn-ui-classic');
    const btnPremium = document.getElementById('btn-ui-premium');
    
    if (btnClassic && btnPremium) {
        if (style === 'classic') {
